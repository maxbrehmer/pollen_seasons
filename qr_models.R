...
qr_1 <- df %>% nest_by(station, lat_name) %>% mutate(model = list(rq(formula = greg_day ~ year_bf2023, data = data, tau = 0.01))) %>% 
  rowwise() %>%
  mutate(`Predicted 2023 1% (QR)` = as.integer(predict(rq(greg_day ~ year_bf2023, data = data, tau = 0.01), newdata = data.frame(year_bf2023 = 50))), 
         `Predicted 1973 1% (QR)` = as.integer(predict(rq(greg_day ~ year_bf2023, data = data, tau = 0.01), newdata = data.frame(year_bf2023 = 0)))) %>%
  dplyr::select(c(-data))

qr_50 <- df %>% nest_by(lat_name, station) %>% mutate(model = list(rq(formula = greg_day ~ year_bf2023, data = data, tau = 0.5))) %>% 
  rowwise() %>%
  mutate(`Predicted 2023 50% (QR)` = as.integer(predict(rq(greg_day ~ year_bf2023, data = data, tau = 0.5), newdata = data.frame(year_bf2023 = 50))), 
         `Predicted 1973 50% (QR)` = as.integer(predict(rq(greg_day ~ year_bf2023, data = data, tau = 0.5), newdata = data.frame(year_bf2023 = 0)))) %>%
  dplyr::select(c(-data))


qr_95 <- df %>% nest_by(lat_name, station) %>% mutate(model = list(rq(formula = greg_day ~ year_bf2023, data = data, tau = 0.95))) %>%
  rowwise() %>%
  mutate(`Predicted 2023 95% (QR)` = as.integer(predict(rq(greg_day ~ year_bf2023, data = data, tau = 0.95), newdata = data.frame(year = 50))), 
         `Predicted 1973 95% (QR)` = as.integer(predict(rq(greg_day ~ year_bf2023, data = data, tau = 0.95), newdata = data.frame(year = 0)))) %>%
  dplyr::select(c(-data))


# Adding regression coefficients to each table
qr_1 <- qr_1 %>% mutate("coefficient 1% (QR)" = model$coefficients["year_bf2023"])
qr_50 <- qr_50 %>% mutate("coefficient 50% (QR)" = model$coefficients["year_bf2023"])
qr_95 <- qr_95 %>% mutate("coefficient 95% (QR)" = model$coefficients["year_bf2023"])
