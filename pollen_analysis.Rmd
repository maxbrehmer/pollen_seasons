...
To begin with, we consider models with the only predictor variable being `date` conditioned on the genus of pollen and the response variable `count`, an estimate of the pollen count. By fitting the models to certain quantiles, our aim is to predict the shift in dates of which an amount of pollen is released relative to the total pollen count of that year.
...
By non-parametric regressional models we refer to models that do not make any assumptions about the functional form of the relationship between the reponse variable and predictor variable(s). The advantage of this approach is that one can capture complex non-linear relationships and is more robust when it comes to dealing with significant outliers in the data.
