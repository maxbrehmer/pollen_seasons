...
Before constructing the models, we can take a glance at what sort of results to expect from the subsequent regression analysis. In figure ? we present a ridge plot containing information about the distribution of observed pollen by date of year. By looking at the average distribution from the first and last 5 years in the dataset respectively, we construct a somewhat consistent average in terms of the state of the pollen season at a given location, reducing the risk of falsely identifying outlier data as patterned occurrences. We can conclude that, in the Stockholm area, the pollen season seemed to begin earlier for all the arboreal pollen species while grass pollen did not appear to show those tendencies. In general the shape of the pollen distribution was not seen to have been altered. A visual analysis of the distribution of yearly pollen release may suggest that the average pollen season now begins earlier by approximately 10-20 days compared to 45 years ago.
...
```{r}
# EQ models
kable(head(joined_eq_1, 5), caption = "Start of pollen season for different species and locations, by most significant shift to earlier dates (EQ model)")
kable(head(joined_eq_1 %>% map_df(rev), 5), caption = "Start of pollen season for different species and locations, by most significant shift to later dates (EQ model)")

kable(head(joined_eq_50, 5), caption = "Peak of pollen season for different species and locations, by most significant shift to earlier dates (EQ model)")
kable(head(joined_eq_50 %>% map_df(rev), 5), caption = "Peak of pollen season for different species and locations, by most significant shift to later dates (EQ model)")

kable(head(joined_eq_95, 5), caption = "End of pollen season for different species and locations, by most significant shift to earlier dates (EQ model)")
kable(head(joined_eq_95 %>% map_df(rev), 5), caption = "End of pollen season for different species and locations, by most significant shift to later dates (EQ model)")
```


```{r}
# QR models
kable(head(joined_qr_1, 5), caption = "Start of pollen season for different species and locations, by most significant shift to earlier dates (QR model)")
kable(head(joined_qr_1 %>% map_df(rev), 5), caption = "Start of pollen season for different species and locations, by most significant shift to later dates (QR model)")

kable(head(joined_qr_50, 5), caption = "Peak of pollen season for different species and locations, by most significant shift to earlier dates (QR model)")
kable(head(joined_qr_50 %>% map_df(rev), 5), caption = "Peak of pollen season for different species and locations, by most significant shift to later dates (QR model)")

kable(head(joined_qr_95, 5), caption = "End of pollen season for different species and locations, by most significant shift to earlier dates (QR model)")
kable(head(joined_qr_95 %>% map_df(rev), 5), caption = "End of pollen season for different species and locations, by most significant shift to later dates (QR model)")
```


### Geographic influence
```{r}
ggplot(data = joined_qr_1, aes(x = Latitude, y = `Coefficient (QR)`, colour = Species)) +
  geom_point() +
  stat_smooth(formula = y ~ x, method="lm", se=FALSE) +
  facet_wrap(~Species) +
  labs(colour = "Species", x = "Latitude", y = "Coefficient")

ggplot(data = joined_qr_50, aes(x = Latitude, y = `Coefficient (QR)`, colour = Species)) +
  geom_point() +
  stat_smooth(formula = y ~ x, method="lm", se=FALSE) +
  facet_wrap(~Species) +
  labs(colour = "Species", x = "Latitude", y = "Coefficient")

ggplot(data = joined_qr_95, aes(x = Latitude, y = `Coefficient (QR)`, colour = Species)) +
  geom_point() +
  stat_smooth(formula = y ~ x, method="lm", se=FALSE) +
  facet_wrap(~Species) +
  labs(colour = "Species", x = "Latitude", y = "Coefficient")
