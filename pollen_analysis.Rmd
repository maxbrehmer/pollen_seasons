...
eq_joined_1 %>% head(6) %>% kable(caption = "Earlier shifted 1% (EQ)")
eq_joined_1_desc %>% head(5) %>% kable(caption = "Later shifted 1% (EQ)")

eq_joined_50 %>% head(6) %>% kable(caption = "Earlier shifted 50% (EQ)")
eq_joined_50_desc %>% head(2) %>% kable(caption = "Later shifted 50% (EQ)")

eq_joined_95 %>% head(6) %>% kable(caption = "Earlier shifted 95% (EQ)")
eq_joined_95_desc %>% head(6) %>% kable(caption = "Later shifted 95% (EQ)")
```
...
### Information about reduced data sets
ESKILSTUNA:
  ALNUS: 45 362
