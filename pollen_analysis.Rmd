...
plot_trend <- ggplot() +
  ...
  stat_quantile(data = trend_data, aes(x = Year, y = greg_day), quantiles = c(0.01)) +
  geom_jitter(data = trend_summarised, aes(x = Year, y = q1, color = "red")) +
  geom_smooth(data = trend_summarised, method = "lm", aes(x = Year, y = q1, color = "red"))

plot_trend
```

[][][] Talk about how the QR method fits quantiles of data under te limit and how an outlying year with lots of observations performs badly especially for these extreme quantiles like 1%. [][][]

```{r}
trend_summarised

df %>% group_by(Year, lat_name, station, latitude) %>%
                summarise(q1 = quantile(greg_day, prob = .01)) %>% filter(station == "Stockholm", lat_name == "Betula")
```
