...
trend_summarised <- df %>% group_by(Year, lat_name, station, latitude, greg_day) %>%
...
plot_trend <- ggplot() +
  ...
  geom_smooth(data = trend_summarised, method = "lm", aes(x = Year, y = greg_day))

plot_trend
```

```{r}
df %>% group_by(Year, lat_name, station, latitude, greg_day) %>%
                summarise(q1 = quantile(greg_day, prob = .01)) %>% filter(station == "Stockholm", lat_name == "Betula")
```
