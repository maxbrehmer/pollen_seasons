...
Comment on geom_density_ridges... (Kernel desnity estimates)
...
QR <- rq(formula = greg_day ~ `Years since 1973`, data = df %>% filter(station == "Stockholm"), tau = 1:99/100)
...
```{r, warning=FALSE}
lm <- lm(data = df %>% filter(station == "Stockholm"), formula = greg_day ~ `Years since 1973`)
...
for (i in seq_along(quantile_levels)) {
  ...
  lm_quantile <- lm(formula = greg_day ~ `Years since 1973`, data = quantile_data)
  ...
  lm_quantile_df_tau[i+i, "term"] <- "Years since 1973"
  lm_quantile_df_tau[i+i, "estimate"] <- lm_quantile_coef[2]
  lm_quantile_df_tau[i+i, "conf.low"] <- lm_quantile_ci["Years since 1973", 1]
  lm_quantile_df_tau[i+i, "conf.high"] <- lm_quantile_ci["Years since 1973", 2]
  lm_quantile_df_tau[i+i, "tau"] <- tau
}

lm_quantile_df_tau <- lm_quantile_df_tau %>%
  mutate(term = replace(term, term == c("Years since 1973"), c("Year")))
```
