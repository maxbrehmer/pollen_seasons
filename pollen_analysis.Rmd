...
Using data provided by the Swedish Museum of Natural History, we are able to perform two different methods, nonparametric quantile regression (QR) and linear regression on empirical quantiles (EQ), to produce predictive linear models on how seasonal starting, peak and ending dates of various species of pollen change over time in a warming climate. Daily counts of 7 species of pollen, collected at 8 different locations across the country form the material for which we perform our research. By analyzing statistical aspects of models using both methods, we found that EQ produces a better fit for determining changes in seasonal distribution of pollen.

Our findings reveal that, among arboreal species, an advance in the start of seasonal pollen release occurs by between $0.35$ and $0.81$ calendar days per year, depending on species. The peaks of pollen intensity advances by $0.26$ to $0.63$ days each year, while the pollen season ends between $0.23$ and $0.49$ days earlier annually. Grass pollen was found to react differently to the arboreal species, as the start and peak dates stay largely unchanged while the ending dates move to later dates. Another phenomenon which we see in our models is that the lengths of pollen seasons increase over time by between $4$ and $25$ days over a period of 50 years. Furthermore, the effects mentioned appear to be intensified at higher latitudes, apart from the extension of seasonal length for grass pollen, which was found to be reduced at more northerly latitudes.
...
Section $\textbf{6. Discussion}$, is divided in to $(\textbf{6.1 Comparison with prior studies})$ and $(\textbf{6.2 Further improvements})$. The former section covers a comparative analysis of our results with other, similar studies. For each of our questions at issue, we may be able obtain a definitive conclusion, in which case we present this in $(\textbf{5. Conclusions})$. If we are not able to obtain a definitive conclusion, a description of what we is missing for us to draw a conclusion follows. Since there are limits to what we can expect to answer in this thesis, we have set aside a section $(\textbf{6.2 Further improvements})$ on which parts of our work is inconclusive and how they can be improved upon.

This paper concludes with the an $\textbf{Appendicies}$ section. Additional or complementary information which the reader may find interesting, but not vital for the results of which we form our conclusions. Each appendix is reffered to by, and refers back to a result or explanation.
...
```{r, fig.cap="Illustration of the pinball loss function.", out.width="300px"}
...
Comparing equation 8 with equation 3, note that we replace the scalar $\mu_{\tau}$ with $x_ib$ in the loss function and minimize over $b$ leading to an estimation of the $\beta$-coefficient which coupled with a vector of data $x$ grants a predictive linear model.
...
kable(shift_by_species %>%
        dplyr::select(Species, `Avg slope (1%)`, `P-value (1%)`, `Adj. R^2 (1%)`, 
                      ...
                      `Slope (season length)`), caption = "Modelled pollen season shift by species averaged over all monitored stations (models with P-values larger than 0.05 are ignored).")

kable(shift_by_species %>%
        dplyr::select(Species, `Estimated start 1973`, `Estimated start 2023`, `Estimated start 2050`, 
                      `Season length 1973`, `Season length 2023`, `Season length 2050`), caption = "Estimated starting dates and seasonal length averaged over all monitored stations (models with P-values larger than 0.05 are ignored).")
```
...
kable(shift_by_species_sthlm %>%
        dplyr::select(Species, `Slope (1%)  `, `P-value (1%)`, `Adj. R^2 (1%)`, 
                      ...
                      `Slope (season length)`), caption = "Modelled pollen season shift by species in Stockholm (models with P-values larger than 0.05 are ignored).")

kable(shift_by_species_sthlm %>%
        dplyr::select(Species, `Estimated start 1973`, `Estimated start 2023`, `Estimated start 2050`, 
                      `Season length 1973`, `Season length 2023`, `Season length 2050`), caption = "Estimated starting dates and seasonal length averaged in Stockholm (models with P-values larger than 0.05 are ignored).")
```
...
The results in this study point towards EQ being a more fitting model than QR when predicting changes in seasonal pollen distributions. Largely, this has to do with the fact that the spread, or variance of dates is fairly low, leading to clustering of extreme observations in the case of abnormally hot or cold years. This can in turn be catastrophic when using nonparametric quantile regression to predict extreme quantiles, as discussed in $\textbf{Section 4.1: Statistical model performance across quantiles}$.

Based on the slopes of our predictive linear models, we conclude that an advance in the seasonal start of pollen release among arboreal species occurs by between $0.35$ and $0.81$ calendar days per year. The seasonal peak in arboreal pollen appears to also be heavily affected by climate change, as an advance of $0.26$ to $0.63$ days per year is observed. The 95th percentile of annual observed pollen was found to not advance as dramatically, leading to longer periods of seasonal activity. Since 1973, season lengths of arboreal pollen have increased by $4$-$20$ days on average, while grass pollen saw an increase of as much as $25$ days over the same 50-year time frame. Grass pollen is the only species to attain a positive slope (pollination occurring later over time) as we see ending dates regress by $0.08$ days annually.

A comparison of seasonal pollen distributions between various locations, as seen in $\textbf{Section 4.3: Geographic influence}$ reveals that the slopes can differ quite dramatically between latitudes. In particular we see that alder, hazel and oak pollen see more dramatic advances at higher latitudes, while herbaceous species appear less affected by climate change further north.

## Section 6: Discussion

## Section 6.1: Comparison with prior studies
...
A substantial amount of our models are not significant at 95% confidence level. The majority of these are for combinations of `species` and `station` that do not have extensive sets of data, leading to less accurate models. In particular, grass pollen and observations from the Umeå laboratory are overrepresented in poorly fitted models. As we remove the contributions of models with high P-values in our analysis to ensure our conclusions are drawn from models with significant outcomes, this does not paint the whole picture of how climate change effects the release of pollen in Sweden since some slope estimations are built on more data than others.
