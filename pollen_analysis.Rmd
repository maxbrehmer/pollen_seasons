...
```{r}
QR <- rq(formula = greg_day ~ year, data = df %>% filter(station == "Stockholm", lat_name == "Alnus"), tau = 1:99/100)
...
plotQR <- QR_tidy %>% 
  ...
  #geom_hline(data = ?, aes(yintercept= `coef(lm)`), lty=1, color="#512ffa", size=1) + 
