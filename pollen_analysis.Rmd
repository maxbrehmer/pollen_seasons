...
plotQR <- QR_tidy %>% 
  ...
  # EQ
  geom_line(data = lm_quantile_df_tau, aes(x = tau, y = estimate, group = term, color="EQ"), linewidth = 0.5, show.legend = FALSE) + # Set color to "EQ"
...
```{r}
# EQ models
kable(head(joined_eq_1 %>% dplyr::select(Species, Location, `Coefficient (EQ)`, `P value (EQ)`, `Predicted 1973 (EQ)`, `Predicted 2023 (EQ)`, `Predicted 2050 (EQ)`), 5), caption = "Start of pollen season for different species and locations, by most significant shift to earlier dates (EQ model)")
kable(head(joined_eq_1 %>% dplyr::select(Species, Location, `Coefficient (EQ)`, `P value (EQ)`, `Predicted 1973 (EQ)`, `Predicted 2023 (EQ)`, `Predicted 2050 (EQ)`) %>% map_df(rev), 5), caption = "Start of pollen season for different species and locations, by most significant shift to later dates (EQ model)")
