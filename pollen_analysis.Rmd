...
Before constructing the models, we can take a glance at what sort of results to expect from the subsequent regression analysis. In figure ? we present a ridge plot containing information about the distribution of observed pollen by date of year. The `geom_density_ridges` function from the `R` library `ggridges` is used to fit a continuous density function to what is, in essence, a histogram of the frequency of pollen on each date.

[][][] CHATGPT [][][]
The densities in geom_density_ridges() are calculated using kernel density estimation (KDE), which is a non-parametric method for estimating the probability density function of a random variable.

KDE estimates the density by placing a kernel (usually a Gaussian or normal distribution) at each data point, and then summing up the contributions from all the kernels to obtain a smooth estimate of the density. The width of the kernel (also known as the bandwidth) determines how much each data point contributes to the density estimate, with larger bandwidths resulting in smoother but less detailed density estimates, and smaller bandwidths resulting in more detailed but noisier density estimates.

In geom_density_ridges(), the KDE is computed separately for each variable, with the bandwidth chosen automatically based on the data using the density() function in R. By default, geom_density_ridges() uses a Gaussian kernel to estimate the densities, but this can be changed using the kernel parameter.

The resulting density estimates are then stacked on top of each other to create the ridgeline plot, with each density plot offset by a small amount along the y-axis to prevent overlap between the densities.

||||||| SOURCES |||||||
The official ggridges package documentation: https://cran.r-project.org/web/packages/ggridges/ggridges.pdf

A tutorial on ridgeline plots in R, which covers the ggridges package and kernel density estimation: https://www.data-to-viz.com/caveat/joyplot.html

A chapter on kernel density estimation from the book "Introduction to Data Science" by Rafael A. Irizarry: https://rafalab.github.io/dsbook/distributions.html#kernel-density-estimation

An article on kernel density estimation from the journal "WIREs Computational Statistics": https://onlinelibrary.wiley.com/doi/abs/10.1002/wics.1486
||||||| SOURCES |||||||
