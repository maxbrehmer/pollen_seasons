...
QR <- rq(formula = greg_day ~ year_bf2023, data = df %>% filter(station == "Stockholm"), tau = 1:99/100)
...
```{r}
QR_tidy <- QR_tidy %>%
  mutate(term = replace(term, term == c("year_bf2023"), c("Year")))
```

```{r, warning=FALSE}
lm <- lm(data = df %>% rename(Year = year_bf2023) %>% filter(station == "Stockholm"), formula = greg_day ~ Year)
...
for (i in seq_along(quantile_levels)) {
  ...
  lm_quantile <- lm(formula = greg_day ~ year_bf2023, data = quantile_data)
  
  # Extract coefficient estimates and confidence intervals
  lm_quantile_coef <- coef(lm_quantile)
  lm_quantile_ci <- confint(lm_quantile)
  ...
  lm_quantile_df_tau[i+i, "term"] <- "year_bf2023"
  lm_quantile_df_tau[i+i, "estimate"] <- lm_quantile_coef[2]
  lm_quantile_df_tau[i+i, "conf.low"] <- lm_quantile_ci["year_bf2023", 1]
  lm_quantile_df_tau[i+i, "conf.high"] <- lm_quantile_ci["year_bf2023", 2]
...
```{r}
# Create a data frame with the results for the current quantile level
#  lm_quantile_df_tau <- data.frame(term = rownames(lm_quantile_coef),
#                                   estimate = lm_quantile_coef,
#                                   conf.low = lm_quantile_ci[, 1],
#                                   conf.high = lm_quantile_ci[, 2],
#                                   tau = tau,
#                                   stringsAsFactors = FALSE)
#  
  # Append the results for the current quantile level to the list
#  lm_quantile_list[[as.character(tau)]] <- lm_quantile_df_tau
#}

# Convert the list of data frames to a single data frame
#lm_quantile_df <- do.call(rbind, lm_quantile_list)

# Convert the term variable to a factor for correct ordering on the plot
#lm_quantile_df$term <- factor(lm_quantile_df$term, levels = unique(lm_quantile_df$term))
...
plotQR <- QR_tidy %>% 
  ...
  geom_line(color="#eb2a3d", size = 0.5) +
  geom_hline(data = ols2, aes(yintercept= `coef(lm)`), lty=1, color="#512ffa", size=0.5) + 
  geom_hline(aes(yintercept = 0), lty=2, color="black", size=0.5) + 
  geom_ribbon(aes(ymin=conf.low, ymax=conf.high), alpha=0.25, fill="#eb2a3d") +
